...
import { GraduationCap, Lock, ShieldCheck, BookOpen, FileCheck2, Database, AlertCircle, User } from 'lucide-react';
...
const StudentLogin: React.FC = () => {
    ...
    const submit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        const result = await login(regNumber, password, 'student');
        setLoading(false);
        if (!result) setError('Invalid registration number or password');
    };

    const fillDemo = () => {
        setRegNumber('TC/2025/001');
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="relative grid lg:grid-cols-2 gap-8 max-w-6xl w-full">
                ...
                <div className="bg-white rounded-3xl shadow-2xl p-8 lg:p-10">
                    ...
                    <h3 className="text-2xl font-bold text-slate-900 mb-2">Student Sign In</h3>
                    <p className="text-slate-500 mb-6">Sign in with your registration number</p>

                    <form onSubmit={submit} className="space-y-4">
                        <div>
                            <label className="text-sm font-medium text-slate-700 mb-1 block">Registration Number</label>
                    ...
                    <div className="mt-6 pt-6 border-t border-slate-200">
                        ...
                        <button onClick={fillDemo} className="w-full text-xs px-2 py-1.5 bg-slate-100 hover:bg-slate-200 rounded">
                            Demo Student (TC/2025/001)
                        </button>
                    </div>
                    ...
                    <div className="mt-4 text-center">
                        <a href="/login/staff" className="text-sm text-indigo-600 hover:text-indigo-700">
                            Staff Login →
                        </a>
                    </div>
                </div>
