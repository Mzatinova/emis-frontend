...
const StudentResultsHistory: React.FC = () => {
    ...
    const currentLevel = [...new Set(groupedByYear[year].map(r => r.level))][0];
    
    // Check if this level appears in any previous session
    const isRepeaterSession = sortedYears.slice(index + 1).some(prevYear => {
        const prevLevel = [...new Set(groupedByYear[prevYear].map(r => r.level))][0];
        return prevLevel === currentLevel;
    });
    
    const allPassed = groupedByYear[year].every(r => r.grade !== 'F' && r.marks !== null);
    const hasFail = groupedByYear[year].some(r => r.grade === 'F');
    
    let statusText = '';
    let statusColor = '';
    
    if (isRepeaterSession && allPassed) {
        statusText = 'RETAKEN - PASSED ✓';
