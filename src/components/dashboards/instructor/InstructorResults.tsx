...
const InstructorResults: React.FC<InstructorResultsProps> = ({ toast, setToast, myAssignedCourses }) => {
    ...
    const existingResultsMap = useMemo(() => {
        ...
        results.forEach(r => {
            if (r.courseId === courseId && r.sessionId === sessionId) {
                map[r.studentId] = r;
            }
        });
        return map;
    }, [results, myCourseIds, sessions]);
    ...
    const handleSaveResult = async (studentId: string) => {
        ...
        const existing = existingResultsMap[studentId];
        const courseId = myCourseIds[0];
        const sessionId = sessions[0]?.id;
        ...
        lines.forEach(line => {
            ...
            if (stu && cou) {
                addResult({
                    ...
                    sessionId: sessions[0]?.id || '', exam: examS ? parseFloat(examS) : null,
                    createdBy: currentUser!.id
                });
                added++;
            }
    ...
    {existing ? (
        ...
        <Badge status="default">Not Entered</Badge>
                                    ...
                                    <td className="px-4 py-3">
    <div className="flex gap-2">
        {/* Save button appears when score is entered and different from saved value */}
        {!isLocked && examValues[s.id] && examValues[s.id] !== (existing?.exam?.toString() || '') && (
            <button
        ...
        {/* Edit button appears for existing saved results */}
        {existing && !isLocked && !examValues[s.id] !== (existing?.exam?.toString() || '') && (
            <button
                onClick={() => {
                    setEditingStudentId(s.id);
                    setExamValues(prev => ({ ...prev, [s.id]: existing?.exam?.toString() || '' }));
                }}
