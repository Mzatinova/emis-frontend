...
//     const { feeStructures, addFeeStructure, updateFeeStructure, deleteFeeStructure } = useRegistration();
//     const [toast, setToast] = useState('');
//     const [feeModal, setFeeModal] = useState(false);
//     const [editingFee, setEditingFee] = useState<any>(null);
//     const [feeForm, setFeeForm] = useState({ programId: '', programName: '', level: 1, amount: 0 });

//     const programs = useMemo(() => {
//         const progMap = new Map();
//         students.forEach(s => {
//             if (s.program && !progMap.has(s.program)) {
//                 progMap.set(s.program, s.program);
//             }
//         });
//         return Array.from(progMap.values()).map(p => ({ id: p, name: p }));
//     }, [students]);

//     const handleAddFee = () => {
//         if (!feeForm.programId || !feeForm.amount) {
...
//         });
//         setFeeModal(false);
//         setFeeForm({ programId: '', programName: '', level: 1, amount: 0 });
//         setToast('Fee structure added');
//     };

//     const handleUpdateFee = () => {
//         if (!editingFee) return;
//         updateFeeStructure(editingFee.id, { amount: feeForm.amount });
//         setFeeModal(false);
//         setEditingFee(null);
//         setToast('Fee structure updated');
//     };
...
//             <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
//                 <Table headers={['Program', 'Level', 'Amount', 'Actions']} rowCount={feeStructures.length}>
//                     {feeStructures.map(fee => (
//                         <tr key={fee.id} className="hover:bg-slate-50">
//                             <td className="px-4 py-3 font-medium">{fee.programName}</td>
//                             <td className="px-4 py-3 text-center">Level {fee.level}</td>
//                             <td className="px-4 py-3 font-medium">K{fee.amount.toLocaleString()}</td>
//                             <td className="px-4 py-3">
//                                 <div className="flex gap-2">
//                                     <button onClick={() => openFeeModal(fee)} className="p-1.5 hover:bg-slate-100 rounded text-slate-600">
//                                         <Eye className="w-4 h-4" />
...
//             <Modal open={feeModal} onClose={() => setFeeModal(false)} title={editingFee ? 'Edit Fee Structure' : 'Add Fee Structure'}>
//                 <form onSubmit={(e) => { e.preventDefault(); editingFee ? handleUpdateFee() : handleAddFee(); }} className="space-y-4">
//                     <Field label="Program" required>
//                         <Select value={feeForm.programId} onChange={e => {
//                             const program = programs.find(p => p.id === e.target.value);
//                             setFeeForm({ ...feeForm, programId: e.target.value, programName: program?.name || '' });
//                         }} disabled={!!editingFee}>
//                             <option value="">Select program</option>
...
//                     </Field>
//                     <Field label="Level" required>
//                         <Select value={feeForm.level} onChange={e => setFeeForm({ ...feeForm, level: parseInt(e.target.value) })} disabled={!!editingFee}>
//                             <option value={1}>Level 1</option>
//                             <option value={2}>Level 2</option>
//                             <option value={3}>Level 3</option>
//                             <option value={4}>Level 4</option>
//                         </Select>
//                     </Field>
//                     <Field label="Amount (K)" required>
//                         <Input type="number" min="0" value={feeForm.amount} onChange={e => setFeeForm({ ...feeForm, amount: parseInt(e.target.value) || 0 })} />
//                     </Field>
//                     <div className="flex justify-end gap-2">
//                         <Button type="button" variant="secondary" onClick={() => setFeeModal(false)}>Cancel</Button>
