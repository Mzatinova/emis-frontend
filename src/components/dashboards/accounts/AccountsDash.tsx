...
const AccountsDash: React.FC<{ active: string }> = ({ active }) => {
  const { students, currentUser } = useEMIS();
  const { getPendingInvoices, verifyPaymentPhysical, invoices, registrations, feeStructures, fetchInvoices } = useRegistration();
  ...
  const handlePhysicalVerify = async (invoiceId: string) => {
    ...
    await fetchInvoices();  // Refresh data after verification
      ...
      <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            ...
            <Table headers={['Student', 'Level', 'Amount', 'Receipt', 'Payment Verified', 'Action']} rowCount={Math.min(pendingInvoices.length, 5)}>
            ...
            <div className="space-y-2">
              {feeStructures.slice(0, 5).map(fee => (
                <div key={fee.id} className="flex justify-between items-center p-2 border-b border-slate-100">
                  <div>
                    <span className="font-medium text-sm">{fee.programName}</span>
