...
//   addResult: (r: Omit<Result, 'id' | 'createdAt' | 'grade' | 'total' | 'status'>) => Promise<void>;
//   updateResult: (id: string, r: Partial<Result>) => Promise<void>;
//   approveResult: (id: string) => Promise<void>;
...
// const computeGrade = (total: number | null): string => {
//   if (total === null || isNaN(total)) return 'INC';

//   const savedScale = localStorage.getItem('grading_scale');
//   if (savedScale) {
//     const gradingScale = JSON.parse(savedScale);
...
//   const refresh = async () => {
//     try {
//       const [usersData, studentsData, sessionsData, coursesData, modulesData, resultsData, auditsData] = await Promise.all([
//         apiRequest('/users').catch(() => ({ data: [] })),
//         apiRequest('/students').catch(() => ({ data: [] })),
//         apiRequest('/sessions').catch(() => ({ data: [] })),
//         apiRequest('/courses').catch(() => ({ data: [] })),
//         apiRequest('/modules').catch(() => ({ data: [] })),
//         apiRequest('/results').catch(() => ({ data: [] })),
//         apiRequest('/audits').catch(() => ({ data: [] })),
//       ]);
...
//     const data = await apiRequest('/modules', 'POST', m);
//     if (data.data) {
//       setModules(prev => [...prev, data.data]);
//     }
//   };

//   const addResult = async (r: Omit<Result, 'id' | 'createdAt' | 'grade' | 'total' | 'status'>) => {
//     const total = (r.ca || 0) + (r.exam || 0);
//     const grade = computeGrade(total);
//     const data = await apiRequest('/results', 'POST', { ...r, total, grade });
//     if (data.data) {
//       setResults(prev => [data.data, ...prev]);
//     }
//   };

//   const updateResult = async (id: string, r: Partial<Result>) => {
//     let updateData = { ...r };
//     if ((r.ca !== undefined || r.exam !== undefined)) {
//       const existing = results.find(x => x.id === id);
//       const ca = r.ca !== undefined ? r.ca : existing?.ca;
//       const exam = r.exam !== undefined ? r.exam : existing?.exam;
//       const total = (ca || 0) + (exam || 0);
//       const grade = computeGrade(total);
//       updateData = { ...updateData, total, grade };
//     }
//     const data = await apiRequest(`/results/${id}`, 'PUT', updateData);
//     if (data.data) {
//       setResults(prev => prev.map(r => r.id === id ? data.data : r));
//     }
//   };
...
//   const approveResult = async (id: string) => {
//     const data = await apiRequest(`/results/${id}/approve`, 'POST');
//     if (data.data) {
//       setResults(prev => prev.map(r => r.id === id ? data.data : r));
//     }
//   };
...
//     const data = await apiRequest('/results/approve-all', 'POST');
//     if (data.data) {
//       setResults(prev => prev.map(r => r.status === 'pending' ? { ...r, status: 'approved' } : r));
//     }
//   };
